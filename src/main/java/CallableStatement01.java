...
public class CallableStatement01 {
    public static void main(String[] args) throws ClassNotFoundException, SQLException {
        ...
        //2st Step: Execute the function
        st.execute(sql1); // returns false


        //3rd step: Call the function
        CallableStatement cst1 = con.prepareCall("{? = call additionF(? , ?) }");

        //4.Step: Use registerOutParameter() method for result container, use set() method for parameters
        cst1.registerOutParameter(1,Types.NUMERIC);
        cst1.setInt(2,6);
        cst1.setInt(3,4);

        //5.Step: Execute to get result for specific values
        cst1.execute();
